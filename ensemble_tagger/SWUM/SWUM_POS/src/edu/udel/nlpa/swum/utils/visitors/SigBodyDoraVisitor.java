...
public class SigBodyDoraVisitor extends SWUMSearchVisitor {
	...
	private String altdir = "dora_results/5edge/sdora25/";
	String pkg;
	...
	public SigBodyDoraVisitor(String q, IJavaProject jp, PrintStream p) {
		super(q, jp, p);
	...
	public SigBodyDoraVisitor(String q, IJavaProject jp, PrintStream p, Set<String> s, double[] maxes) {
		this(q, jp, p);
		seeds = s;
		max_sig = maxes[0];
		max_body = maxes[1];
	...
	private void findCallers(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IMethod m : c) {
				...
				double sig_callee = sig_alpha * callee.getSigs() / max_sig + body_alpha * callee.getBowScore() / max_body;
				double sig_caller = sig_alpha * caller.getSigs() / max_sig + body_alpha * caller.getBowScore() / max_body;
	...
	private void findFields(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IField m : c) {
				...
				double sig_caller = sig_alpha * current.getSigs() / max_sig + 
							body_alpha * current.getBowScore() / max_body;
				double combo = sig_alpha * use.getSigs() / max_sig + body_alpha * use.getBowScore() / max_body;
		...
		if (c.size() > 0) {
			...
			for (IMember m : c) {
				if (m instanceof IMethod) {
					...
					double sig = sig_alpha * caller.getSigs() / max_body + 
						body_alpha * caller.getBowScore() / max_body;
					double fscore = sig_alpha * field.getSigs() / max_sig + body_alpha * field.getBowScore() / max_body ;
	...
	private void findCallees(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IMethod m : c.keySet()) {
				...
				double sig_callee = sig_alpha * callee.getSigs() / max_sig + body_alpha * callee.getBowScore() / max_body;
				double sig_caller = sig_alpha * current.getSigs() / max_sig + body_alpha * current.getBowScore() / max_body;
