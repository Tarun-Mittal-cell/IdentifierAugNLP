...
public class SWUMDoraVisitor extends SWUMSearchVisitor {
	...
	private String altdir = "dora_results/1edge/rdora/";
	String pkg;
	
	private int    t_distance = 1; // 10
	...
	public SWUMDoraVisitor(String q, IJavaProject jp, PrintStream p) {
		super(q, jp, p);
	...
	public SWUMDoraVisitor(String q, IJavaProject jp, PrintStream p, Set<String> s, double[] maxes) {
		this(q, jp, p);
		seeds = s;
		max_sig = maxes[0];
		max_body = maxes[1];
...
//						visited.get(h)[0] += sig_callee;
//						if (visited.get(h)[0] > 1) {
//							DoraNode d = createOrGetNode(pg, m, edgesAway + 1, caller, NodeType.C);
//							pg.addEdge(d, sd, EdgeType.CONNECT, s);
//						}
//					} else
//						visited.put(h, new double[]{sig_callee});
//				}
	...
	private void findCallees(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IMethod m : c.keySet()) {
				...
				/*} else if (sd.getType() == NodeType.I && act_callee > t_very_greedy) {
							...
							s = act_callee;*/
				} else if (act_callee > t_very_greedy && sig_callee > 0) { // 0.6? 0.5? && name > 0?
					type = NodeType.S;
					if (m.isBinary())
						type = NodeType.LS;
					forward = true;
