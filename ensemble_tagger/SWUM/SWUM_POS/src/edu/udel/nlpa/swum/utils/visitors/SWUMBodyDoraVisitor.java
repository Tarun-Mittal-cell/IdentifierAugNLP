...
public class SWUMBodyDoraVisitor extends SWUMSearchVisitor {
	...
	private String altdir = "dora_results/5edge/rdora75/";
	String pkg;
	...
	private double body_alpha = 0.75;
	...
	public SWUMBodyDoraVisitor(String q, IJavaProject jp, PrintStream p) {
		super(q, jp, p);
	...
	public SWUMBodyDoraVisitor(String q, IJavaProject jp, PrintStream p, Set<String> s, double[] maxes) {
		this(q, jp, p);
		seeds = s;
		max_sig = maxes[0];
		max_body = maxes[1];
	...
	private void findCallers(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IMethod m : c) {
				...
				double sig_callee = sig_alpha * callee.getSwums() / max_sig + body_alpha * callee.getBowScore() / max_body;
				double sig_caller = sig_alpha * caller.getSwums() / max_sig + body_alpha * caller.getBowScore() / max_body;
	...
	private void findFields(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IField m : c) {
				...
				double sig_caller = sig_alpha * current.getSwums() / max_sig + 
							body_alpha * current.getBowScore() / max_body;
				double combo = sig_alpha * use.getSwums() / max_sig + body_alpha * use.getBowScore() / max_body;
		...
		if (c.size() > 0) {
			...
			for (IMember m : c) {
				if (m instanceof IMethod) {
					...
					double sig = sig_alpha * caller.getSwums() / max_sig + 
						body_alpha * caller.getBowScore() / max_body;
					double fscore = sig_alpha * field.getSwums() / max_sig + body_alpha * field.getBowScore() / max_body ;
	...
	private void findCallees(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IMethod m : c.keySet()) {
				...
				double sig_callee = sig_alpha * callee.getSwums() / max_sig + body_alpha * callee.getBowScore() / max_body;
				double sig_caller = sig_alpha * current.getSwums() / max_sig + body_alpha * current.getBowScore() / max_body;
				double act_callee = sig_callee;
				double sig_cle = callee.getSwums();
...
//				current.setScore(sig_caller);
				...
				if (!sd.getElement().isBinary()) {
					...
					callee.gatherScores(m, actuals);
					act_callee = sig_alpha * callee.getSwums() / max_sig + body_alpha * callee.getBowScore() / max_body;
				}
