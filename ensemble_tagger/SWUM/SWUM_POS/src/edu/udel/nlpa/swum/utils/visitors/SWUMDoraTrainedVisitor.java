...
public class SWUMDoraTrainedVisitor extends SWUMSearchVisitor {
	...
	public SWUMDoraTrainedVisitor(String q, IJavaProject jp, PrintStream p) {
		super(q, jp, p);
	...
	public SWUMDoraTrainedVisitor(String q, IJavaProject jp, PrintStream p, Set<String> s, double[] maxes) {
		this(q, jp, p);
		seeds = s;
		max_sig = maxes[0];
		max_body = maxes[1];
			...
			if (d.getElement() instanceof IField) { // for each field
				...
				for (DoraEdge e : d.getInEdges().values()) {
					...
					if ( e.getEdgeType() == EdgeType.PLAIN &&
						...
						if (!doubleCheckHasOtherLinks(e.getFromNode(), pg)) {
	...
	private void findFamily(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IMember m : c) {
				...
				if (!ContextBuilder.getSignature(m).matches(sd.getSignature())) {
					DoraNode d;
