...
import edu.udel.nlpa.swum.scores.CalculateSigSearchScoreData;
import edu.udel.nlpa.swum.utils.context.ContextBuilder;

public class LogLinearSigBodyDoraVisitor extends SWUMSearchVisitor {
	...
	private String altdir = "dora_results/5edge/lydora25/";
	String pkg;
	...
	private double body_alpha = 0.25;
	private double sig_alpha = 1 - body_alpha;
	...
	public LogLinearSigBodyDoraVisitor(String q, IJavaProject jp, PrintStream p) {
		super(q, jp, p);
	...
	public LogLinearSigBodyDoraVisitor(String q, IJavaProject jp, PrintStream p, Set<String> s, double[] maxes) {
		this(q, jp, p);
		seeds = s;
		max_sig = maxes[0];
		max_body = maxes[1];
		max_combo = maxes[2];
		max_fcombo = maxes[3];
		//max_body = max_sig; // just normalize by sigs
		max_body = swum.getContains().size();
		max_sig = max_body;
	}
...
//		}
	...
	private void explore(DoraNode sd, int edgesAway, ProgramGraph pg) {
		...
		if (sd.getElement() instanceof IMethod) {
			...
			if (sd.shouldGoForward() && !sd.wentForward()) {
				// callees
				findCallees(sd, edgesAway, pg, (CalculateSigSearchScoreData)current);
	...
	private AbstractScoreData getScore(DoraNode sd) {
		...
		if (sd.getScore() == null) {
			if (sd.getElement() instanceof IMethod) {
			current = new CalculateSigSearchScoreData(dora, sig, swum, name, bow);
			((CalculateSigSearchScoreData)current).
	...
	private void findCallers(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			if (debug) out.print("\nCallers:\n");
			for (IMethod m : c) {
				CalculateSigSearchScoreData caller = new CalculateSigSearchScoreData(dora, sig, swum, name, bow);
				caller.gatherScores(m);
				
				
				double sig_callee = logit(sig_alpha * callee.getSigs() / max_sig + body_alpha * callee.getBowScore() / max_body - intercept);
				double sig_caller = logit(sig_alpha * caller.getSigs() / max_sig + body_alpha * caller.getBowScore() / max_body - intercept);
				
				
//				callee.setScore(sig_callee);
//				caller.setScore(sig_caller);
				...
				double lavg = 0d;
				if (callee.getSwums() > 0 && caller.getSwums() > 0) {
					lavg = 0.5 * (callee.getSwums() + caller.getSwums());
				}
				...
				if (sd.getType() == NodeType.K) {
					...
					if (sig_caller > t_very_greedy) { // 0.7? 0.51? // t_conserv?
						if (m.isBinary()) {
							...
							type = NodeType.M;
							forward = true; // maybe only if more conserv?
							backward = true;
						}
						s = sig_caller;
	...
	private void findFields(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IField m : c) {
				...
				double sig_caller = logit(sig_alpha * current.getSigs() / max_sig + 
							body_alpha * current.getBowScore() / max_body - intercept);
				double combo = logit(sig_alpha * use.getSigs() / max_sig + body_alpha * use.getBowScore() / max_body - intercept);
				
//				use.setScore(combo);
//				current.setScore(sig_caller);
		...
		if (c.size() > 0) {
			...
			for (IMember m : c) {
				if (m instanceof IMethod) {
					...
					double sig = logit(sig_alpha * caller.getSigs() / max_body + 
						body_alpha * caller.getBowScore() / max_body - intercept);
					double fscore = logit(sig_alpha * field.getSigs() / max_sig + body_alpha * field.getBowScore() / max_body  - intercept);

//					field.setScore(fscore);
//					caller.setScore(sig);
	...
	private void findCallees(DoraNode sd, int edgesAway, ProgramGraph pg,
		...
		if (c.size() > 0) {
			...
			for (IMethod m : c.keySet()) {
				...
				CalculateSigSearchScoreData callee = new CalculateSigSearchScoreData(dora, sig, swum, name, bow);
				callee.gatherScores(m, actuals);
				
				double sig_callee = logit(sig_alpha * callee.getSigs() / max_sig + body_alpha * callee.getBowScore() / max_body - intercept);
				double sig_caller = logit(sig_alpha * current.getSigs() / max_sig + body_alpha * current.getBowScore() / max_body - intercept);
				double act_callee = sig_callee;
				double sig_cle = callee.getSigs();
				
//				callee.setScore(sig_callee);
//				current.setScore(sig_caller);
				// actuals have no place in node score
//				if (!sd.getElement().isBinary()) {
//					actuals = getActualTheme((IMethod)sd.getElement(), m);
//
//					callee.gatherScores(m, actuals);
//					act_callee = sig_alpha * callee.getSigs() / max_body + body_alpha * callee.getBowScore() / max_body;
//				}
				...
				if (isO && act_callee > 0) {
					...
					s = 1d;
				} else if (act_callee > t_very_greedy && sig_cle > 0) { // 0.6? 0.5? && name > 0?
					type = NodeType.S;
					if (m.isBinary())
						type = NodeType.LS;
					forward = true;
...
//				} else if (act_callee > 0) {
//					//if (handleVisited(m, callee)) {
//					if (handleVisited(m, sd)) {
//						DoraNode d = createOrGetNode(pg, m, edgesAway + 1, callee, NodeType.C);
//						pg.addEdge(sd, d, EdgeType.CONNECT, s);
//						pg.addEdge(sd, visited.get(DoraNode.getHandle(m)), EdgeType.CONNECT, s);
//					}
//				}
